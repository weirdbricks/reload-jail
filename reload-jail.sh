...
create_jail(){
	echo "creating jail1"
  	ezjail-admin create jail1 $ipaddress > /dev/null 2>&1
	...
	awk '{ gsub("#ListenAddress 0.0.0.0","ListenAddress '$ipaddress'" ) ; print }' /usr/jails/jail1/etc/ssh/sshd_config > /usr/jails/jail1/etc/ssh/sshd_config_edited
...
#	mv /usr/jails/jail1/etc/ssh/sshd_config_edited /usr/jails/jail1/etc/ssh/sshd_config
	...
	cat ~/.ssh/id_rsa.pub >> /usr/jails/jail1/root/.ssh/authorized_keys
	cat ~/.ssh/windows.pub >> /usr/jails/jail1/root/.ssh/authorized_keys
...
delete_jail(){
	...
	echo "deleting old key from known_hosts"
        sed -i.'' '/'$ipaddress'/d' ~/.ssh/known_hosts
